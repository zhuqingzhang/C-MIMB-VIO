...
void StateHelper::add_kf_nuisance(State *state,Keyframe *kf)
{
    ...
    {
      state->_Cov_nuisance.block(new_loc,new_loc,total_size,total_size)=kf->P;
    }
    ...
    if(state->_seq_Keyframes.find(kf->sequence)==state->_seq_Keyframes.end())
    {
      vector<Keyframe*> kfs;
      kfs.push_back(kf);
      state->_seq_Keyframes.insert({kf->sequence,kfs});
    }
    else
    {
      state->_seq_Keyframes[kf->sequence].push_back(kf);
    }
...
bool StateHelper::add_map_transformation(State *state,Matrix<double,7,1> pose_vio_to_map,int seq_id) {
    ...
    if(seq_id==-1)
      ...
      state->transform_vio_to_maps[seq_id]=pose_tranform;
    // Type *new_clone = nullptr;
