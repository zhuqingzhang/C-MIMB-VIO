...
RosVisualizer::RosVisualizer(ros::NodeHandle &nh, VioManager* app, Simulator *sim) : _nh(nh), _app(app), _sim(sim) {
    ...
    pub_transform_1 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/transform1", 2);
    pub_transform_2 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/transform2", 2);
    pub_transform_3 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/transform3", 2);
    pub_transform_4 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/transform4", 2);
    ROS_INFO("Publishing: %s", pub_poseimu_rect.getTopic().c_str());
    pub_local_relative_transform = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ov_msckf/local_relative_transform",2);      
    ...
    pub_mappath_1 = nh.advertise<nav_msgs::Path>("/ov_msckf/mappath_1", 2);
    pub_mappath_2 = nh.advertise<nav_msgs::Path>("/ov_msckf/mappath_2", 2);
    pub_mappath_3 = nh.advertise<nav_msgs::Path>("/ov_msckf/mappath_3", 2);
    pub_mappath_4 = nh.advertise<nav_msgs::Path>("/ov_msckf/mappath_4", 2);
    ...
    pub_match_keyframe_location_seq1 = nh.advertise<sensor_msgs::PointCloud2>("/ov_msckf/keyframe_location_seq1", 3000);
    pub_match_keyframe_location_seq2 = nh.advertise<sensor_msgs::PointCloud2>("/ov_msckf/keyframe_location_seq2", 3000);
    pub_match_keyframe_location_seq3 = nh.advertise<sensor_msgs::PointCloud2>("/ov_msckf/keyframe_location_seq3", 3000);
    pub_match_keyframe_location_seq4 = nh.advertise<sensor_msgs::PointCloud2>("/ov_msckf/keyframe_location_seq4", 3000);
    // ROS_INFO("Publishing: %s", pub_match_location.getTopic().c_str());
...
void RosVisualizer::publish_state() {
    ...
    maps_keyframe_positions_in_local.clear();
    if(state->_options.use_multi_map)
    {
    for(int i=0;i<state->set_transforms.size();i++)
    {
      if(state->set_transforms[i])
      {
        Eigen::Matrix3d R_VIOtoMap =state->transform_vio_to_maps[i]->Rot();
        Eigen::Vector3d p_VIOinMap = state->transform_vio_to_maps[i]->pos();
        Eigen::Quaterniond q_VIOtoMap(R_VIOtoMap);
        size_t id=state->transform_vio_to_maps[i]->id();
        Eigen::Matrix<double,6,6> cov_transform_i = state->_Cov.block(id,id,6,6);
        geometry_msgs::PoseWithCovarianceStamped relative_transform;
        relative_transform.header=poseIinM.header;
        relative_transform.pose.pose.orientation.x=q_VIOtoMap.x();
        relative_transform.pose.pose.orientation.y=q_VIOtoMap.y();
        relative_transform.pose.pose.orientation.z=q_VIOtoMap.z();
        relative_transform.pose.pose.orientation.w=q_VIOtoMap.w();
        relative_transform.pose.pose.position.x=p_VIOinMap.x();
        relative_transform.pose.pose.position.y=p_VIOinMap.y();
        relative_transform.pose.pose.position.z=p_VIOinMap.z();
        Eigen::Matrix<double,6,6> cov_transform_=Matrix<double,6,6>::Identity(); //in the form of pos,ori
        cov_transform_.block(0,0,3,3) = cov_transform_i.block(3,3,3,3);
        cov_transform_.block(3,3,3,3) = cov_transform_i.block(0,0,3,3);
        cov_transform_.block(0,3,3,3) = cov_transform_i.block(3,0,3,3);
        cov_transform_.block(3,0,3,3) = cov_transform_i.block(0,3,3,3);
        for(int r=0; r<6; r++) {
            for(int c=0; c<6; c++) {
                relative_transform.pose.covariance[6*r+c] = cov_transform_(r,c);
            }
        }

        vector<Keyframe*> keyframe_poses = state->_seq_Keyframes[i];
        vector<Vector3d> transformed_poses;
        transformed_poses.clear();
        for(int id_kf=0;id_kf<keyframe_poses.size();id_kf++)
        {
          Vector3d kf_position_in_local = R_VIOtoMap.transpose() * (keyframe_poses[id_kf]->_Pose_KFinWorld->pos()-p_VIOinMap);
          transformed_poses.push_back(kf_position_in_local);
        }
        maps_keyframe_positions_in_local.insert({i,transformed_poses});

        
        bool show_map_traj = _app->params.load_map_traj;
        if(i==0)
        {
          pub_transform_1.publish(relative_transform);
          state->of_keyframe_positions[i]<<to_string(timestamp_inI)<<" ";
          for(int j=0;j<transformed_poses.size();j++)
          {
            state->of_keyframe_positions[i]<<to_string(keyframe_poses[j]->time_stamp)<<" "<<to_string(transformed_poses[j].x())<<" "<<to_string(transformed_poses[j].y())
            <<" "<<to_string(transformed_poses[j].z())<<" ";
          }
          state->of_keyframe_positions[i]<<endl;
          
          if(show_map_traj)
          {
            vector<geometry_msgs::PoseStamped> poses;
            // cout<<"seq"<<i<<", the number of kfs: "<<_app->map_poses[i].size()<<endl;

            for(int j=0;j<_app->map_poses[i].size();j++)
            {
              PoseJPL* pose_jpl=_app->map_poses[i][j];
              if(pose_jpl==nullptr)
              {
                continue;
              }
              // cout<<"pose R: "<<pose_jpl->Rot()<<endl;
              // cout<<"j vs size: "<<j<<" "<<_app->map_poses[i].size()<<endl;
              Matrix3d R_before = pose_jpl->Rot();
              Vector3d t_before = pose_jpl->pos();
              Matrix3d R_after = R_VIOtoMap .transpose() * R_before;
              Vector3d t = R_VIOtoMap.transpose() * (t_before - p_VIOinMap);
              Quaterniond q(R_after);
              geometry_msgs::PoseStamped pose;
              pose.header.stamp=ros::Time(timestamp_inI);
              pose.header.seq=pose_seq_map1;
            ...
            nav_msgs::Path arrMap;
            arrMap.header.stamp = ros::Time::now();
            arrMap.header.seq = pose_seq_map1;
            arrMap.header.frame_id = "global";
            for(size_t j=0; j<poses.size(); j+=std::floor(poses.size()/16384.0)+1) {
                arrMap.poses.push_back(poses.at(j));
            }
            pub_mappath_1.publish(arrMap);
        ...
        {
          pub_transform_2.publish(relative_transform);
          state->of_keyframe_positions[i]<<to_string(timestamp_inI)<<" ";
          for(int j=0;j<transformed_poses.size();j++)
          {
            state->of_keyframe_positions[i]<<to_string(keyframe_poses[j]->time_stamp)<<" "<<to_string(transformed_poses[j].x())<<" "<<to_string(transformed_poses[j].y())
        ...
        {
          pub_transform_3.publish(relative_transform);
          state->of_keyframe_positions[i]<<to_string(timestamp_inI)<<" ";
          for(int j=0;j<transformed_poses.size();j++)
          {
            state->of_keyframe_positions[i]<<to_string(keyframe_poses[j]->time_stamp)<<" "<<to_string(transformed_poses[j].x())<<" "<<to_string(transformed_poses[j].y())
        ...
        {
          pub_transform_4.publish(relative_transform);
          state->of_keyframe_positions[i]<<to_string(timestamp_inI)<<" ";
          for(int j=0;j<transformed_poses.size();j++)
          {
            state->of_keyframe_positions[i]<<to_string(keyframe_poses[j]->time_stamp)<<" "<<to_string(transformed_poses[j].x())<<" "<<to_string(transformed_poses[j].y())
    ...
    if(state->_options.use_multi_map)
    {
    if(state->set_transforms[state->destination_kf->sequence])
    {
      Keyframe* kf=state->destination_kf;
      int seq = kf->sequence;
      Eigen::Matrix3d R_VIOtoMap =state->transform_vio_to_maps[seq]->Rot();
      Eigen::Vector3d p_VIOinMap = state->transform_vio_to_maps[seq]->pos();
      Eigen::Matrix3d R_KFtoMap = kf->_Pose_KFinWorld->Rot();
      Eigen::Vector3d p_KFinMap = kf->_Pose_KFinWorld->pos();
      Eigen::Matrix3d R_ItoVIO = state->_imu->Rot().transpose();
      Eigen::Vector3d p_IinVIO = state->_imu->pos();
      Eigen::Matrix3d R_KFtoVIO = R_VIOtoMap.transpose() * R_KFtoMap;
      Eigen::Vector3d p_KFinVIO = R_VIOtoMap.transpose()*(p_KFinMap - p_VIOinMap);
      Eigen::Matrix3d R_KFtoI = R_ItoVIO.transpose() * R_KFtoVIO;
      Eigen::Vector3d p_KFinI = R_ItoVIO.transpose() * (p_KFinVIO-p_IinVIO);
      Quaterniond q_KFtoI(R_KFtoI);
      geometry_msgs::PoseWithCovarianceStamped local_relative_transform;
      local_relative_transform.header=poseIinM.header;
      local_relative_transform.pose.pose.orientation.x=q_KFtoI.x();
      local_relative_transform.pose.pose.orientation.y=q_KFtoI.y();
      local_relative_transform.pose.pose.orientation.z=q_KFtoI.z();
      local_relative_transform.pose.pose.orientation.w=q_KFtoI.w();
      local_relative_transform.pose.pose.position.x=p_KFinI.x();
      local_relative_transform.pose.pose.position.y=p_KFinI.y();
      local_relative_transform.pose.pose.position.z=p_KFinI.z();
      Eigen::Matrix<double,6,6> cov_transform_=Matrix<double,6,6>::Identity(); //in the form of pos,ori
      for(int r=0; r<6; r++) {
          for(int c=0; c<6; c++) {
              local_relative_transform.pose.covariance[6*r+c] = cov_transform_(r,c);
          }
      }
      pub_local_relative_transform.publish(local_relative_transform);  
...
void RosVisualizer::publish_features() {
    ...
    auto iter=maps_keyframe_positions_in_local.begin();
    for(;iter!=maps_keyframe_positions_in_local.end();iter++)
    {
      sensor_msgs::PointCloud2 cloud_match_keyframe;
      cloud_match_keyframe.header.frame_id = "global";
      cloud_match_keyframe.header.stamp = ros::Time::now();
      cloud_match_keyframe.width  = 3*iter->second.size();
      cloud_match_keyframe.height = 1;
      cloud_match_keyframe.is_bigendian = false;
      cloud_match_keyframe.is_dense = false; // there may be invalid points

      // Setup pointcloud fields
      sensor_msgs::PointCloud2Modifier modifier_match(cloud_match_keyframe);
      modifier_match.setPointCloud2FieldsByString(1,"xyz");
      modifier_match.resize(3*iter->second.size());

      // Iterators
      sensor_msgs::PointCloud2Iterator<float> out_x_match_keyframe(cloud_match_keyframe, "x");
      sensor_msgs::PointCloud2Iterator<float> out_y_match_keyframe(cloud_match_keyframe, "y");
      sensor_msgs::PointCloud2Iterator<float> out_z_match_keyframe(cloud_match_keyframe, "z");

      // Fill our iterators
      // cout<<"************************size!!!!!!!: "<<map_match_location.size()<<"*********************"<<endl;
      int seq = iter->first;
      vector<Vector3d> map_keyframe_location = iter->second;
      for(const auto &pt : map_keyframe_location) {
          *out_x_match_keyframe = pt(0); ++out_x_match_keyframe;
          *out_y_match_keyframe = pt(1); ++out_y_match_keyframe;
          *out_z_match_keyframe = pt(2); ++out_z_match_keyframe;
      }

      // Publish
      if(seq==0)
      {
        pub_match_keyframe_location_seq1.publish(cloud_match_keyframe);
      }
      else if(seq==1)
      {
        pub_match_keyframe_location_seq2.publish(cloud_match_keyframe);
      }
      else if(seq==2)
      {
        pub_match_keyframe_location_seq3.publish(cloud_match_keyframe);
      }
      else if(seq==3)
      {
        pub_match_keyframe_location_seq4.publish(cloud_match_keyframe);
      }
    }
