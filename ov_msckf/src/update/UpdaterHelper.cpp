...
bool UpdaterHelper::get_feature_jacobian_kf4_multi_map(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                             Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                             std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
    ...
    for (auto const& pair : feature.keyframe_matched_obs) {
        for(auto const& match : pair.second)
        {
            ...
            int seq_id = state->get_kfdataBase()->get_keyframe(kf_id)->sequence;
            assert(state->set_transforms[seq_id]);
            PoseJPL *transform = state->transform_vio_to_maps[seq_id];
              //we also need to add transformation between map and vio
            if(map_hx.find(transform)==map_hx.end())
            {
                map_hx.insert({transform,total_hx});
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
        ...
        //get the transform between the VIO(G) to World(W)
        PoseJPL* transform = state->transform_vio_to_maps[seq_id];
        Eigen::Matrix<double,3,3> R_GtoW_linp = transform->Rot_linp();
        Eigen::Matrix<double,3,1> p_GinW_linp = transform->pos_linp();
        Eigen::Matrix<double,3,3> R_GtoW_fej = transform->Rot_fej();
        Eigen::Matrix<double,3,1> p_GinW_fej = transform->pos_fej();
        // if(state->_options.trans_fej)
        // {
        //     R_GtoW_linp=R_GtoW_fej;
        //     p_GinW_linp=p_GinW_fej;
        // }
        Eigen::Matrix<double,3,3> R_GtoW = transform->Rot();
        Eigen::Matrix<double,3,1> p_GinW = transform->pos();
        ...
        for(int i=0;i<feature.timestamps[0].size();i++)
        {
            ...
            H_x.block(2*c,map_hx[transform],2,transform->size()).noalias() = dz_dpfg*dpfg_dxtrans;
            // }
        ...
        //now compute reprojecting the 3d point in anchor frame to other kf (note that other kf has different reference than anchor frame)
        for(int i = 1;i<keyframe_id.size();i++)
        {
            kf_id = keyframe_id[i];
            kf_feature_id = keyframe_feature_id[i];
            Keyframe* keyframe = state->get_kfdataBase()->get_keyframe(kf_id);
            int seq_id_i=keyframe->sequence;
            assert(keyframe != nullptr);
            PoseJPL* kf_w=state->_clones_Keyframe[kf_id];
            Eigen::Matrix<double,3,3> R_KFtoWi = kf_w->Rot();
            Eigen::Matrix<double,3,1> p_KFinWi = kf_w->pos();

            PoseJPL* transform_i = state->transform_vio_to_maps[seq_id_i];
            Eigen::Matrix<double,3,3> R_GtoWi_linp = transform_i->Rot_linp();
            Eigen::Matrix<double,3,1> p_GinWi_linp = transform_i->pos_linp();
            Eigen::Matrix<double,3,3> R_GtoWi_fej = transform_i->Rot_fej();
            Eigen::Matrix<double,3,1> p_GinWi_fej = transform_i->pos_fej();
        // if(state->_options.trans_fej)
        // {
        //     R_GtoW_linp=R_GtoW_fej;
        //     p_GinW_linp=p_GinW_fej;
        // }
            Eigen::Matrix<double,3,3> R_GtoWi = transform_i->Rot();
            Eigen::Matrix<double,3,1> p_GinWi = transform_i->pos();

            Eigen::Matrix<double,3,3> R_WtoWi = R_GtoWi*R_GtoW.transpose();
            Eigen::Matrix<double,3,1> p_WinWi = p_GinWi - R_WtoWi*p_GinW;
            Eigen::Matrix<double,3,3> R_WtoWi_linp = R_GtoWi_linp*R_GtoW_linp.transpose();
            Eigen::Matrix<double,3,1> p_WinWi_linp = p_GinWi_linp - R_WtoWi_linp*p_GinW_linp;
            Eigen::Matrix<double,3,3> R_WtoWi_fej = R_GtoWi_fej*R_GtoW_fej.transpose();
            Eigen::Matrix<double,3,1> p_WinWi_fej = p_GinWi_fej - R_WtoWi_fej*p_GinW_fej;

            Eigen::Vector3d p_FinWi = R_WtoWi*p_FinW + p_WinWi;
            Eigen::Vector3d p_FinWi_linp = R_WtoWi_linp*p_FinW_linp + p_WinWi_linp;
            Eigen::Vector3d p_FinWi_fej = R_WtoWi_fej*p_FinW_linp + p_WinWi_fej;
            ...
            Eigen::Vector3d p_FinKF = R_KFtoWi.transpose()*(p_FinWi-p_KFinWi);
            Eigen::Vector3d p_FinKF_linp=R_KFtoWi.transpose()*(p_FinWi_linp-p_KFinWi);
            Eigen::Vector3d p_FinKF_fej = R_KFtoWi.transpose()*(p_FinWi_fej-p_KFinWi);
            cout<<"p_FinKF: "<<p_FinKF.transpose()<<" "<<p_FinKFOb.transpose()<<endl;
//           sleep(1);
            ...
            Matrix<double,3,3> dpfk_dpfwi = R_KFtoWi.transpose();

            Matrix<double,3,3> dpfwi_dpfw = R_WtoWi_linp;

            Matrix<double,3,3> dpfw_dpfanchor = R_AnchorKFtoW;

            H_f.block(2*c,0,2,H_f.cols()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dpfwi*dpfwi_dpfw*dpfw_dpfanchor;

            Matrix<double,3,6> dpfw_dxnanchor=Matrix<double,3,6>::Zero();
            dpfw_dxnanchor.block(0,0,3,3)=skew_x(R_AnchorKFtoW*p_FinAnchorKF_linp);
            dpfw_dxnanchor.block(0,3,3,3)=Matrix<double,3,3>::Identity();

            H_n.block(2*c, map_hn[anchor_kf_w],2,anchor_kf_w->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dpfwi*dpfwi_dpfw*dpfw_dxnanchor;

            Matrix<double,3,6> dpfk_dxn=Matrix<double,3,6>::Zero();
            dpfk_dxn.block(0,0,3,3)=-R_KFtoWi.transpose()*skew_x(p_FinWi_linp-p_KFinWi);
            dpfk_dxn.block(0,3,3,3)=-R_KFtoWi.transpose();

            H_n.block(2*c, map_hn[kf_w],2,kf_w->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dxn;

            Matrix<double,3,6> dpfwi_dtrans=Matrix<double,3,6>::Zero();
            dpfwi_dtrans.block(0,0,3,3) = R_WtoWi_linp*skew_x(p_GinW_linp-p_FinW_linp);
            dpfwi_dtrans.block(0,3,3,3) = -R_WtoWi_linp;

            H_x.block(2*c,map_hx[transform],2,transform->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dpfwi*dpfwi_dtrans;

            Matrix<double,3,6> dpfwi_dtransi=Matrix<double,3,6>::Zero();
            dpfwi_dtransi.block(0,0,3,3) = skew_x(R_WtoWi_linp*(p_FinW_linp-p_GinW_linp));
            dpfwi_dtransi.block(0,3,3,3) = Matrix3d::Identity();

            H_x.block(2*c,map_hx[transform_i],2,transform_i->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dpfwi*dpfwi_dtransi;
...
bool UpdaterHelper::get_feature_jacobian_kf5_multi_map(State *state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
                                             Eigen::MatrixXd &H_n,Eigen::MatrixXd &H_x, Eigen::VectorXd &res, std::vector<Type *> &n_order,
                                             std::vector<Type *> &x_order) {
    // Total number of measurements for this feature
    int total_meas = 0;
...
//           sleep(1);
            ...
            Matrix<double,3,3> dpfwi_dpfw = R_WtoWi_fej;
            ...
            Matrix<double,3,6> dpfk_dxn=Matrix<double,3,6>::Zero();
            dpfk_dxn.block(0,0,3,3)=-R_KFtoWi.transpose()*skew_x(p_FinWi_fej-p_KFinWi);
            dpfk_dxn.block(0,3,3,3)=-R_KFtoWi.transpose();

            H_n.block(2*c, map_hn[kf_w],2,kf_w->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dxn;

            Matrix<double,3,6> dpfwi_dtrans=Matrix<double,3,6>::Zero();
            dpfwi_dtrans.block(0,0,3,3) = R_WtoWi_fej*skew_x(p_GinW_fej-p_FinW_linp);
            dpfwi_dtrans.block(0,3,3,3) = -R_WtoWi_fej;

            H_x.block(2*c,map_hx[transform],2,transform->size()).noalias() = dzkf_dzn*dzn_dpfk*dpfk_dpfwi*dpfwi_dtrans;

            Matrix<double,3,6> dpfwi_dtransi=Matrix<double,3,6>::Zero();
            dpfwi_dtransi.block(0,0,3,3) = skew_x(R_WtoWi_fej*(p_FinW_linp-p_GinW_fej));
            dpfwi_dtransi.block(0,3,3,3) = Matrix3d::Identity();
...
void UpdaterHelper::get_feature_jacobian_loc_multi_map(State *state, UpdaterHelperFeature &feature,
                                             Eigen::MatrixXd &H_x, Eigen::VectorXd &res,
                                             std::vector<Type *> &x_order)
{
    int total_meas = 0;
    ...
    for(auto const& pair: feature.keyframe_matched_obs)
    {
      ...
      for(auto const& match: pair.second)
      {
        ...
        //as each keyframe has a 3d point represented in itself, we choose the first kf as anchor kf,
        //and reproject its 3d point into kfs and current frame
      for(int j=0;j<keyframe_id.size();j++)
      {

        
        
        double kf_id = keyframe_id[j];
        Keyframe* anchor_kf = state->get_kfdataBase()->get_keyframe(kf_id);
        int seq_id = anchor_kf->sequence;
        assert(anchor_kf != nullptr);
        size_t kf_feature_id = keyframe_feature_id[j];
